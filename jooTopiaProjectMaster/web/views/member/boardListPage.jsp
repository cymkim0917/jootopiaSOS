			...
			<div class="col-sm-7" id="tableArea" style="text-align: center;">
				...
				<div id="searchArea">
					<table class="table" id="myTable">
						<tr>
							<td colspan="2" rowspan="2"><br /> <br /> <label for="">게시글
									카테고리</label> <select name="btype" id="btype">
									<option value="1">FAQ</option>
									<option value="2">공지</option>
									<option value="3">1:1문의</option>
									<option value="4">후기</option>
									<option value="5">매입요청</option>
							</select><br /> <label for="">검색 유형</label> <select name="searchType"
								id="searchType">
									<option value="1">제목</option>
									<option value="2">내용</option>
									<option value="3">내용+제목</option>
							</select> <label for="">검색 내용</label> <input type="text" name="searchText"
								id="searchText" /> &nbsp; <input type="button" value="검색"
								onclick="searchBoard()" /></td>
				...
				<div id="myTable">
					...
					for(int i=0; i<list.size(); i++){ %>
							<tr>
								<td><label><%= boardIndex %>
										<% boardIndex++; %></label> <input type="hidden" name="bid"
									value="<%= list.get(i).getbId() %>" /></td>
								<% int num = list.get(i).getbType();  
							switch(num){
							case 3: %>
								<td><a
									href="<%= request.getContextPath() %>/selectQaA.do?num=<%=list.get(i).getbId() %>"><%= list.get(i).getbTitle() %></a></td>
								<% break; 
							case 1: %>
								<td><a
									href="<%= request.getContextPath() %>/SelectOneFaqTotalList.do?num=<%=list.get(i).getbId() %>"><%= list.get(i).getbTitle() %></a></td>
								<% break; case 2: %>
								<td><a
									href="<%= request.getContextPath() %>/selectNoticeOne.do?num=<%=list.get(i).getbNo() %>"><%= list.get(i).getbTitle() %></a></td>
								<%  break; case 4: %>
								<td><a
									href="<%= request.getContextPath() %>/selectOneReview.do?num=<%=list.get(i).getbId() %>"><%= list.get(i).getbTitle() %></a></td>
								<% break; case 5: %>
								<td><a href="#"><%= list.get(i).getbTitle() %></a></td>
								<% break;} %>
								<td><label for=""><%= list.get(i).getbDate() %></label></td>
								<td><label for=""><%= list.get(i).getbCount() %></label>
								<td>
									<% 
							   String bType = "";
								switch(num){
