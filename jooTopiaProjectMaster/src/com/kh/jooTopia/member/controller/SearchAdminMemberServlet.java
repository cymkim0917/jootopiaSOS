...
/**
 * Servlet implementation class SearchAdminMemberServlet
 */
@WebServlet("/searchAdminMember.do")
public class SearchAdminMemberServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SearchAdminMemberServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		sMember.setUserId(sUserId);
		sMember.setUserName(sName);
		sMember.setPhone(sPhone);
		
		if(sDate != "") {
			Date scDate = Date.valueOf(sDate);
			sMember.setUserDate(scDate);
		}
		sMember.setAddress(sAddress);
		sMember.setEmail(sEmail);
		
		
		ArrayList<Member> list = new MemberAdminService().searchMember(pageInfo,sMember);
