...
@WebServlet("/insertPersonDeny.do")
public class InsertPCDDenyAdminServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public InsertPCDDenyAdminServlet() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = new PurchaseDetailService().insertPCDpersonDeny(pcd);
		
		String page = "";
		if(result > 0) {
			request.setAttribute("hmap", new PurchaseAdminService().selectPurchaseOne(pcid));
			page = "/views/admin/storage/buyAccept.jsp";
		}else {
